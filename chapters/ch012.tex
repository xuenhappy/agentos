...
\[
\boxed{
\text{学习，是认知结构改变其表示形式、功能位置、时间尺度与承载关系的过程。}
}
\]
...
\[
\boxed{
\text{能力成熟以后，认知负担不是凭空消失，而是改变了自己的承载位置。}
}
\]
...
\[
\boxed{
\text{真正具有连续性的不是复杂度值，而是认知结构及其因果谱系。}
}
\]
...
\[
\boxed{
\text{认知结构在可塑功能拓扑上的生成、激活、变换、迁移与稳定。}
}
\]
...
\resizebox{.96\linewidth}{!}{$\displaystyle
\boxed{
\text{有效认知结构通常不会在能力成熟时简单消失，而会发生压缩、抽象、合并、外化或承载位置改变。}
}
$}
\]
...
\[
\boxed{
\text{学习是认知结构改变自身表示、功能位置、时间尺度和承载关系的过程。}
}
\]
...
\[
\boxed{
\text{``一个认知结构，在智能体整个生命周期中究竟如何改变自己的表示与承载位置？''}
}
\]
