...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{进入什么}
\rightarrow
\text{如何表示}
\rightarrow
\text{何时处理}
...
\begin{proposition*}
必须进入工作记忆的结构，也不应以比当前任务需要更高的自由度存在。
\end{proposition*}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle SimultaneousComplexity
\rightarrow
TemporalDepth.\)}
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle AgentCSOActivationStructure
:
Parallel
\rightarrow
Sequential.\)}
...
\begin{proposition*}
当重要结构不能继续删除和压缩时，不要要求它们同时存在；让它们在时间中逐步展开。
\end{proposition*}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \mathcal M_B:
AgentCSO(C,B_i)
\rightarrow
AgentCSO(C,B_j).\)}
\end{statementbox}
...
\begin{proposition*}
如果某个结构不值得持续占据核心认知，就应该为它寻找更合适的承载位置，而不是让工作记忆永远为已经解决的问题付费。
\end{proposition*}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Admission
\rightarrow
Compression
\rightarrow
Temporalization
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Selection
\rightarrow
Representation
\rightarrow
ActivationSchedule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth,height=.38\textheight,keepaspectratio]{images/agentos-theory-system-map.png}
\caption{从模型能力到持续主体的 AgentOS 理论全景}
\end{figure}
