...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle D_{\mathrm{eff}}\uparrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{快速局部自治}
\leftrightarrow
\text{缓慢全局治理}.\)}
\end{statementbox}
...
\begin{statementbox}
随着有效耦合范围扩大，稳定智能通常需要把快速闭环局部化，把跨域变量提升到更慢的治理尺度。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{为不同类型的结构选择合适的 }\tau_i.\)}
\end{statementbox}
