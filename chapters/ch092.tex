...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth,height=.38\textheight,keepaspectratio]{images/agent-lifecycle-development-cycle.png}
\caption{Agent 生命周期阶段与慢变量形成}
\end{figure}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{生命周期成熟}
\neq
\text{运行时间增加},\)}
\end{statementbox}
而更接近
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{这个身体}
+
\text{这个世界}
+
\text{这些经历}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{稳定主体}
+
\text{长期任务}
+
\text{现实责任}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle StructuralReorganization
\rightarrow
WorldVerification.\)}
\end{statementbox}
