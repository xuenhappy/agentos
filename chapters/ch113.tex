...
\[
\boxed{
\text{智能是一个在有限物理与计算条件下，由多个不同时间尺度闭环相互嵌套、相互约束并通过现实反馈持续演化的动力过程。}
}
\]
...
\[
\boxed{
\text{智能是在多尺度功能拓扑上生成、表示、组合、迁移、压缩、稳定和重新组织认知结构对象的持续过程。}
}
\]
...
\[
\boxed{
\text{智能是一类能够在自身内部形成关于外部结构、关系、未来以及自身状态之结构表示的动力系统。}
}
\]
...
\[
\boxed{
\text{智能是一类利用内部结构表示构造可能未来，并以这些表示参与现实行动选择，从而改变未来外部结构的动力系统。}
}
\]
...
\[
\boxed{
\text{成熟智能是一类能够通过现实反馈改变自身内部结构的表示、时间尺度、功能承载和组织拓扑，使过去经验逐渐沉降为未来认知条件的自重组动力系统。}
}
\]
...
\[
\boxed{
\text{宇宙中的一个物理子系统形成了关于其他宇宙结构的内部有效表示。}
}
\]
