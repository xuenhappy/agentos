...
\begin{definition*}
AHC 是一个由有限维连续状态、独立时间常数、非线性相互作用和不确定性描述共同构成的中尺度内部调制动力系统。
\end{definition*}
...
\begin{statementbox}
为持续智能定义一组随任务和主体状态变化的可生存、可计算、可恢复区域，并让认知活动在这些区域附近运行。
\end{statementbox}
...
\begin{definition*}
Threat 是潜在损失、不可逆性、时间压力和主体脆弱性经过时间积分后形成的连续控制压力。
\end{definition*}
...
\begin{definition*}
Agent 在当前环境、目标、资源和风险条件下，为未来认知与行动准备分配的连续激活水平。
\end{definition*}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle EnergyDrive=\text{当前可持续投入能力}\)}
\end{statementbox}
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Fatigue=\text{过去持续投入在当前状态中的累计代价}\)}
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle SocialAffinity
=
\text{关系持续与协作接近的动力倾向}\)}
\end{statementbox}
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Trust
=
...
\begin{statementbox}
它把“我不知道”转换成“这个不知道现在值得投入多少资源去解决”。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle AHC\text{ 是压缩动力层，而不是语义推理层。}\)}
\end{statementbox}

从控制理论角度看，AHC 可以视为一个动态 pre-controller state；从认知科学角度，它与内稳态和神经调制存在结构可比性；从 AgentOS 本身看，它更基础地承担了一个时间尺度转换任务：
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle FastSignals
\rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Affective-likeFailure
\not\Rightarrow
SemanticPathology.\)}
\end{statementbox}
必须先检查动态稳定性。
...
\begin{statementbox}
内部状态如何形成、如何衰减、如何耦合、是否可恢复，以及是否仍受现实校准。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{异质瞬时影响}
\rightarrow
\text{连续主体相关状态}
\rightarrow
\text{认知调制}\)}
