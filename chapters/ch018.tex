...
需要首先说明，本章包含三个必须严格区分的知识层次。第一，量子力学、量子场论、统计物理、重整化群、有效理论、退相干等属于现有物理学提供的理论背景；第二，将不同尺度上的稳定有效结构统一称为 CSO，属于本书提出的结构化解释框架；第三，将宇宙微观动力学描述为“量子比特海”、将智能看成宇宙局部形成的自表征结构，则属于用于建立统一认识论的理论模型与哲学性抽象，而不是现代物理学已经证明的事实。
...
本章的核心命题可以预先概括为：
\begin{statementbox}
宇宙不是由一组彼此孤立的对象堆积而成，而可以在不同观察尺度上呈现为一系列具有不同稳定性、关系结构和有效动力学的 CSO 层级。
\end{statementbox}
...
\begin{statementbox}
智能是宇宙结构演化形成的一类特殊结构动力学，而不是宇宙之外的抽象计算过程。
\end{statementbox}
...
\begin{statementbox}
宇宙拥有一个高维量子动力学基底，而我们所看到的宏观世界是该基底在不同尺度上的稳定有效结构。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{高维微观动力学}
\rightarrow
\text{相关结构}
\rightarrow
\text{稳定自由度}
...
\begin{statementbox}
重整化思想为 CSO 的“尺度有效性”提供数学启发，而 CSO 理论进一步把这种结构语言扩展到生命、认知与智能。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle PossibleFutureRepresentation
\rightarrow
PresentAction
\rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Quantum
\rightarrow
Matter
\rightarrow
Gravity
...
\begin{statementbox}
它能够通过内部与集体表征，有选择地重新组织自身及周围的物质结构。
\end{statementbox}
...
\begin{statementbox}
什么变化使一个普通 CSO 开始成为 Agent 型 CSO？
\end{statementbox}
...
\begin{statementbox}
Agent 是一种内部能够形成具有因果作用的“关于结构的结构”的宇宙结构。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Internal
\leftrightarrow
External.\)}
\end{statementbox}
...
\begin{statementbox}
它是否仍然处于 Agent 可恢复、可调用、可验证的因果网络中。
\end{statementbox}
...
\begin{statementbox}
Agent-CSO 是 Universe-CSO 中出现的一类二阶结构：它作为物理结构存在，同时又在 Agent 的功能闭环中指向并承载其他结构。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \mathcal Q
\rightarrow
CSO
\rightarrow
Agent
...
\begin{statementbox}
一个具有量子性质、高维关联结构和持续动力学的基础状态空间隐喻。
\end{statementbox}
...
\begin{statementbox}
在特定条件和历史路径中，宇宙动力学允许产生能够维持、复制、表示和主动控制的高阶局部结构。
\end{statementbox}
...
\begin{statementbox}
不是复杂度作为某种实体迁移，而是认知结构改变自己的实现和承载坐标。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Dynamics
\rightarrow
Representation
\rightarrow
...
\begin{proposition*}
任何 CSO 都只能在特定空间尺度、时间尺度和有效动力学中被定义。
\end{proposition*}

第二，\textbf{多尺度生成原则}：

\begin{statementbox}
高层 Universe-CSO 是底层自由度经长期耦合、粗粒化和稳定化形成的有效结构，而不是与底层物理独立存在的第二世界。
\end{statementbox}

第三，\textbf{表征跃迁原则}：

\begin{statementbox}
Agent 的特殊性不在于单纯拥有更多自由度，而在于其内部形成了关于其他 CSO 以及自身 CSO 的、能够参与未来控制的结构。
\end{statementbox}

第四，\textbf{双重本体原则}：

\begin{statementbox}
Agent-CSO 一方面是宇宙中的真实物理结构，另一方面又在 Agent 的功能闭环中承载关于另一结构的表示关系。
\end{statementbox}
