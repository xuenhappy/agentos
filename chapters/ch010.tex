...
\caption{认知结构对象的关系图形态}
\end{figure}
\begin{introduction}[本章导航]
  \item 从信息到结构：智能真正处理的对象
  \item CSO 的基本定义与认知边界
  \item 对象、关系、目标与约束型 CSO
  \item CSO 系统、事件与未来可能性
\end{introduction}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle RawInformation
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle RepresentationOfFuture
\rightarrow
PresentAction.\)}
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle C_R^{rule}
=
Condition
\Rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Event
\rightarrow
CausalPattern
\rightarrow
Rule.\)}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle SingleTransition
\rightarrow
StableDependency
\rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle FunctionalSelfRepresentation
\not\Rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Value
\rightarrow
GoalPreference
\rightarrow
ActionSelection.\)}
...
\begin{statementbox}
the structure that cognition is about,
\end{statementbox}

而不是：

\begin{statementbox}
the exact data structure used to store it.
\end{statementbox}
...
\begin{statementbox}
同一个 CSO，在一个具体 Agent 中究竟以什么形式存在？
\end{statementbox}
