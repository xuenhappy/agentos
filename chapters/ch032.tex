...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth,height=.38\textheight,keepaspectratio]{images/tce.png}
\caption{TCE 原理示意图}
\end{figure}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \mathrm{TCE}:
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{Cognition}
    \rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle HighGain
    +
    ObservationDelay
    \Rightarrow
    OscillationRisk.\)}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Sensitivity
    \leftrightarrow
    Stability\)}
\end{statementbox}
...
\begin{statementbox}
智能不仅需要状态，还需要关于状态的控制变量。
\end{statementbox}
进一步：
\begin{statementbox}
成熟智能不是始终进行最强认知，而是能够根据自身与现实状态决定应该以何种强度、范围、深度和节奏进行认知。
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \mathrm{SPC}
    \longrightarrow
    \mathrm{TCE}
    \longrightarrow
