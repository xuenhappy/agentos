...
\begin{statementbox}
在核心功能关系基本稳定的条件下，怎样使一个人工智能体成为持续存在的认知主体？
\end{statementbox}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=.9\textwidth,height=.38\textheight,keepaspectratio]{images/agentos-fixed-topology-architecture.png}
\caption{固定拓扑 AgentOS 的最小工程框架}
\end{figure}
\begin{introduction}[本章导航]
  \item 固定功能拓扑的研究边界
  \item 持续主体与可塑内部动力学
  \item 生命周期连续性与认知运行时
  \item AgentOS 与多智能体框架的区别
\end{introduction}
...
\begin{proposition*}
Stabilize the carrier before allowing the carrier to redesign itself.
\end{proposition*}
...
\begin{statementbox}
How much developmental intelligence can emerge on a stable functional skeleton?
\end{statementbox}
...
\begin{statementbox}
Persistent causal history under a stable governance boundary.
\end{statementbox}
...
\begin{statementbox}
A persistent agent must carry its own past into its future.
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{Topology Stability}
\not\Rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{Internal Structural Plasticity}
\not\Rightarrow
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \mathbf{x}(t)
\rightarrow
\mathbf{x}(t+\Delta t),
\qquad
\mu_t
...
\begin{conclusion}
Session AI answers from a context;
...
\begin{statementbox}
How do multiple computational agents cooperate on a task?
\end{statementbox}

而 AgentOS 首先研究：

\begin{statementbox}
How does one persistent subject manage many simultaneous cognitive processes without losing identity continuity?
\end{statementbox}
...
\begin{statementbox}
Multi-Self Internal Architecture.
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle Observe
\rightarrow
Admit
\rightarrow
Interpret
...
\begin{definition*}
A persistent runtime for the governance of intelligence resources and cognitive structure.
\end{definition*}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \mathcal{W}
\rightarrow
Observation
\rightarrow
\mathcal{X}_C
...
\begin{statementbox}
Persistent memory cannot mean persistent activation.
\end{statementbox}
...
\begin{statementbox}
\centering
\adjustbox{max width=.96\linewidth}{\(\displaystyle \text{Persistent Agent Requirement}
\rightarrow
...
\begin{statementbox}
一个持续存在的主体，怎样把正在发生的现在，转化为自己的过去，并进一步把过去转化为生成未来的结构？
\end{statementbox}
